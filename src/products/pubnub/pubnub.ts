...
export async function listPubnubChannelGroup (channelGroup: string): Promise<ListChannelsResponse> {
    client = getClient()
...
export async function waitUntilSubscribed (grantChannel: string, recipient_id: string) {
    while(true) {
        await sleep(1000);
